...
If we assume $\mathbf{u}^\epsilon = O(1)$, then neglecting the $\epsilon^2$ and $\epsilon$ in the anisotropic equations (\ref{ANSC1}) - (\ref{ANSC8}), we formally arrive to the following hydrostatic Navier-Stokes equations (i.e., primitive equations) combined with pollution:
%
...
The weak form of the anisotropic system (\ref{ANSC1}) - (\ref{ANSC8}) is as follows.
...
As we mentioned before, one of the most fundamental tools in this proof is the energy inequality. If we derive it for the anisotropic system (\ref{ANSC1}) - (\ref{ANSC8}), we obtain that for a.e. $t \in [0,T]$ the following holds:
%
\begin{equation}
\begin{split}
...
\begin{proof}
The spatial weak form of the anisotropic system (\ref{ANSC5}) is
%
\begin{equation}
