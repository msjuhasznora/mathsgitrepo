...
%
\begin{remark}
If we wanted to use $C^\epsilon(x,y,z) = c$ with some $c$ constant value, we could make a change of variables subtracting that background flow and arrive back to the same mathematical problem with $C$ being zero.
\end{remark}
...
Finally, the boundary conditions to describe the anisotropic system (\ref{ANSC1}) - (\ref{ANSC7}) can be summarised as follows:
...
If we assume $\mathbf{u}^\epsilon = O(1)$, then neglecting the $\epsilon^2$ and $\epsilon$ in the anisotropic equations (\ref{ANSC1}) - (\ref{ANSC7}), (\ref{ANSC8}), we formally arrive to the following hydrostatic Navier-Stokes equations (i.e., primitive equations) combined with pollution:
%
...
& \partial_1 C = \sigma_1, \partial_2 C = \sigma_2, \partial_3 C = 0 \text{ on } \Gamma_G \times (0,T), \\
& \boldsymbol{\sigma} = (\sigma_1, \sigma_2, 0) \in L^2(0,T; \Gamma_G)\\
\end{split}
\end{equation}
%
...
We will omit the proof of the existence of weak solutions (see Definition \ref{definitionanisotropicWF}.) for the anisotropic system (\ref{ANSC1}) - (\ref{ANSC7}), (\ref{ANSC8}) since it follows by standard finite-dimensional Galerkin approximation.
...
As we mentioned before, one of the most fundamental tools in this proof is the energy inequality. If we derive it for the anisotropic system (\ref{ANSC1}) - (\ref{ANSC7}), (\ref{ANSC8}), we obtain that for a.e. $t \in [0,T]$ the following holds:
%
\begin{equation}
\begin{split}
...
\end{equation}
%
for almost any $x \in \Omega$ and all $\xi \in \mathbb{R}^3$. Moreover we require $M_{ij} \in L^\infty (\Gamma)$ for any $i,j = 1,2,3.$
...
\begin{definition}
Given a source term $S = \delta$ or $S \in L^2(\Omega),$ find $U = (\mathbf{u}_H, C),$ that satisfies the hypothesis of Definition \ref{definitionhydrostaticWF} in the space dimensions, moreover
