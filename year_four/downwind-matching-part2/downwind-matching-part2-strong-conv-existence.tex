...
It does in the application process of $\theta,$ where $u$ denotes $u = \partial_z v,$ and $ \theta = \nabla_H \cdot u + R_1 T.$ In their case this turns out to be a function definition that has full $H^2$ regularities, because when the $L_1 = - \frac{1}{R_1} \Delta_H - \frac{1}{R_2} \partial_z^2$ full Laplacian and a time derivative are applied to the function, then the horizontal temperature Laplacians from $\nabla_H \cdot u$ and $R_1 T$ cancel each other.
...
Thus, in our case we can not keep $\theta$'s definition including the $T$ function --- on the other hand, splitting $T$ off of $\theta$ also alters the proof significantly. The structure described on p58-p60 works in a simultaneous way for $T$ and $v.$ The general result $(4.10)$ is used later also for $T.$ If (4.10) does not include $T$ anymore, then for $T$ we need to create a solution on its own.
