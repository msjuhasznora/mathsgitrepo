...
We propose the strong convergence of $(\mathbf{u}_h^\epsilon, u_3^\epsilon, c^\epsilon)$ over the unbounded $[0, \infty)$ domain, with $\epsilon$ convergence rate and a non-uniform bound in time.
...
Let's have ${\mathbf{u}_h}_0 \in H^3$ and $\epsilon < \epsilon_0,$ where $\epsilon_0$ is the limit specified by Proposition 5.3 in the Li-Titi article.
Starting from this, we have

\begin{enumerate}

  \item Global unique existence of $(\mathbf{u}_h^\epsilon, u_3^\epsilon)$ with the following uniform bound for the difference:
  
  \[ \leq C \epsilon^2\]
  
  \item Based on the previous point, the existence of the extended $(\mathbf{u}_h^\epsilon, u_3^\epsilon, c^\epsilon)$ solution follows from identical steps as the existence of the velocity part.

\end{enumerate}

What still needs to be checked, but probably holds:

\begin{enumerate}

  \item Existence of the $(\mathbf{u}_h, u_3, c)$ strong solutions of the PEs expressed in downwind-matching coordinates.
...
Revising the calculations (12sept2019) on the paper, we have 
...
\begin{enumerate}
  ...
  \item The existence and regularities of the (u,c) strong solution globally of (E-PEs). \todo{details}  We have the existence of both $(u, T)$ from the Cao-Titi 3D wellposedness article with the difference that there they have $\partial_z p + T = 0,$ while for the concentration function we do not have this relation.
\end{enumerate}
...
\section{Rumble, might be useful for later}
...
The difference is the term $ \int_0^{T^*} C \diff \tau,$ which, when ${T^*} = \infty,$ is not bounded. However, for a finite $T^*$ time it is ok, $ \int_0^{T^*} C \diff \tau$ is finite.
