...
\subsection{\textcolor{orange}{The source term}}

For this case of strong convergence, we use the following versions of source functions detailed below. We use mollified sources in order to fit the strong solution scheme.

Let $\delta_\epsilon$ be the following approximation of the $\delta$ distribution,
...
$$ s^\epsilon = \delta^\epsilon \star \varphi, \quad s = \delta \star \varphi, \quad \text{and} \quad S^\epsilon = s^\epsilon - s.$$
...
Multiplying the equation by $- \Delta C^\epsilon$ results only $\epsilon \norm{\partial_{11} C^\epsilon}$ on the left-hand side, i.e. the full, $\epsilon$-free Laplacian is missing. We will take the following different strategy where the Laplacians are not all brought to the left-hand side.

\subsection{Description}

Let's have ${\mathbf{u}_h}_0 \in H^3(\Omega)$ (and as a consequence $\mathbf{u}_0 \in H^2(\Omega)$) and $\epsilon < \epsilon_0,$ where $\epsilon_0$ is the limit specified by Proposition 5.3 in the Li-Titi article. Then we have the following important ingredients needed for estimating the difference function $C^\epsilon.$

\textbf{Cornerstone existence results and regularities:}

\begin{enumerate}

  \item \textbf{Existence for (E-PEs).} Global unique existence of the $(\mathbf{u}_h, u_3, c)$ strong solution of the (E-PEs) expressed in downwind-matching coordinates: in Cao-Li-Titi's paper it has been proved, a difference is that we do not have the $c$-analogous version of the $\partial_z p$---$T$ relationship. The file regarding existence shows the details and necessary modifications.
  ...
  solution function. (This result was used to prove estimates for $c$, but after all probably it is sufficient what we know about $c$ based on the fact that it is a strong solution, and thus is in $H^2.$)
...
\subsection{Estimate for $c$.} \label{estimateforc}

The $H^2$ regularity of $c$ is the consequence of the existence of the global $(u_h, u_3, c)$ strong solution, as described by Cao, Li and Titi for a slightly different system --- i.e. it comes from the definition of $c$ and the existence result. (However it is probably possible to show directly that the $H^2(\Omega)$ norm of $c$ is bounded using the $H^4$ regularity (\ref{H4_uh}) of the $u_h$ solution function.)

\subsection{$c^\epsilon \to c$ in $L^\infty H^1$ and $\nabla_d c^\epsilon \to \nabla_d c$ in $L^2 H^1$ (estimating $\nabla C^\epsilon$ and $\nabla \nabla_d C^\epsilon$)}

 It seems that whatever regularity we might need for $C^\epsilon$ here in this coordinate system, it will be based and built on the $\partial_t C^\epsilon$ term, because from the Laplacians we won't have a complete term as the $\epsilon$ multiplier is there.
...
\textbf{Step 1: estimate for $I_1(i), i = 1,2,3.$}
...
Now we estimate $I_1(j)$ for $j = 2,3.$ Here we can simply use

\begin{equation}
  \begin{split}
    & I_1(2), I_1(3) \leq \norm{U_i^\epsilon}_{L^\infty} \norm{\partial_i C^\epsilon}_2 \norm{\partial_{jj}C^\epsilon}_2 \\
    & \leq \zeta \norm{\partial_{jj} C^\epsilon}_2^2 + \frac{1}{\zeta} \| U_h^\epsilon \| _ {H^3} ^ 2 \norm{\nabla C^\epsilon}_2^2.
  \end{split}
\end{equation}

\textbf{Step 2: estimate for $I_2(i), i=1,2,3.$}
...
\begin{equation}
  \begin{split}
  & I_2(2), I_2(3) = \int_\Omega U^\epsilon \nabla c \partial_{jj} C^\epsilon \leq \norm{U^\epsilon}_{L^\infty} \norm{\nabla c}_2 \norm{\partial_{jj} C^\epsilon}_2 \\
  & \leq \zeta \norm{\partial_{jj} C^\epsilon}_2^2 + \frac{1}{\zeta} \norm{U_h^\epsilon}_{H^3}^2 \norm{\nabla c}_2^2,
  \end{split}
\end{equation}

where $j = 2,3.$

As described in subsection \ref{estimateforc}, the $\norm{c}_{H^2}$ norm is finite for the case of an $H^3$-regular initial horizontal velocity ${u_h}_0,$ which we assumed.

\textbf{Step 3: estimate for $I_3(i), i=1,2,3.$}

Due to their structure, computations for $\int_\Omega u \nabla C^\epsilon \partial_{ii} C^\epsilon$ are analogous to those seen in \textbf{Step 1} for $ \int_\Omega U^\epsilon \nabla C^\epsilon \partial_{ii} C^\epsilon.$

Using the same tools, i.e. the Holder, Ladyzhenskaya and Young inequalities, and the result that
...
\begin{equation}
    I_3(1) \leq C \norm{u_1}_{H^3} \norm{\partial_1 C^\epsilon}_2^2 + \zeta \norm{\nabla \partial_j C^\epsilon}_2^2 + \frac{1}{\zeta} \norm{u_h}_{H^3}^2 \norm{\nabla C^\epsilon}_2^2,
\end{equation}
...
\begin{equation}
  I_3(2), I_3(3) \leq \norm{u_i}_{L^\infty} \norm{\partial_i C^\epsilon}_2 \norm{\partial_{jj}C^\epsilon}_2 \leq \zeta \norm{\partial_{jj} C^\epsilon}_2^2 + \frac{1}{\zeta} \| u_h \| _ {H^3} ^ 2 \norm{\nabla C^\epsilon}_2^2.
\end{equation}

\textbf{Step 4: estimate for $I_4(i), i = 1,2,3$}
...
Since here we do have an $\epsilon$ in front of the Laplacian $ \norm{\partial_{ii} C^\epsilon}_2^2,$ it can be merged into the left-hand side later even for $i = 1.$

\begin{remark}
Note that the $\epsilon$ in $\epsilon K_1 C \norm{\partial_{11} c}_2^2$ will be the convergence rate. If we wanted to have $\epsilon^2$ instead, it would mean that we need to take the $\epsilon$ off of $\partial_{11} C^\epsilon$ (to put it on $\norm{\partial_{11} c}_2^2$) and thus, also take off one derivative off of $\partial_{11} C^\epsilon,$ which would result to the appearance of $\norm{c}_{H^3}.$ Since from the $H^3$ initial data ${u_h}_0$ we can only arrive to an $H^4$-regular solution function $u_h,$ that is sufficient only to guarantee the $H^2$ regularity of $c,$ but not $H^3.$ Increasing the initial velocity's regularity could achieve an $\epsilon^2$ convergence rate, but we are interested in proving a convergence result assuming the minimal needed regularity for the initial horizontal velocity.
\end{remark}

\textbf{\textcolor{orange}{Step 5: estimate for $I_5 (i).$}}
...
\begin{equation}
  \begin{split}
    ...
    & \leq C \int_\Omega \bigg ( (\delta^\epsilon - \delta ) \star \partial_i \varphi \bigg ) ^2 \diff x + \textcolor{orange}{C \norm{\nabla C^\epsilon}_2^2}.
  \end{split}
\end{equation}

The $\norm{\nabla C^\epsilon}_2^2$ norm can be left unchanged, the next step is to estimate the first term. We denote $\Phi = \partial_i \varphi \in C^\infty_0.$ 
...
\begin{equation}
  \begin{split}
    ...
    & \leq C \epsilon^2 \norm{\nabla \Phi}_{L^\infty}^2 \leq \textcolor{orange}{C \epsilon^2}.
  \end{split}
\end{equation}
...
Using the results from the previous subsection, after merging the $\zeta \cdot$ second order terms to the left-hand side we have 

\begin{equation}
  \begin{split}
    & \frac{1}{2} \frac{\diff}{\diff t} \norm{\nabla C^\epsilon}_2^2 + \epsilon K_1 \norm{\partial_{11} C^\epsilon}_2^2 + K_2 \norm{\partial_{22} C^\epsilon}_2^2 + K_3 \norm{\partial_{33} C^\epsilon}_2^2 + C \norm{\partial_{ij} C^\epsilon}_2^2 \\
    & \leq C \textcolor{blue}{\norm{U_H^\epsilon}_{H^3}} \norm{\nabla C^\epsilon}_2^2 + C \norm{U_H^\epsilon}_{H^3}^2 \norm{\nabla C^\epsilon}_2^2 \\
    & + C \textcolor{blue}{\norm{u_H}_{H^3}} \norm{\nabla C^\epsilon}_2^2 + C \norm{u_H}_{H^3}^2 \norm{\nabla C^\epsilon}_2^2 + \textcolor{orange}{C \norm{\nabla C^\epsilon}_2^2} \\
    & + \textcolor{green}{C \norm{U_H^\epsilon}_{H^3}^2} + \norm{c}_{H^2}^2 \norm{\nabla C^\epsilon}_2^2 \\
    & + \textcolor{green}{C \norm{U_H^\epsilon}_{H^3}^2 \norm{\nabla c}_2^2} \\
    & + \textcolor{green}{\epsilon K_1 \norm{\partial_{11} c}_2^2} + \textcolor{orange}{C \epsilon^2},
  \end{split}
\end{equation}
...
\[ \int_0^T \epsilon C \norm{\partial_{11} c}_2^2 + C \norm{U_H^\epsilon}_{H^3}^2 \norm{\nabla c}_2^2 + C \norm{U_H^\epsilon}_{H^3}^2 + \textcolor{orange}{C \epsilon^2} \diff \tau \leq \epsilon C + \epsilon^2 C + \textcolor{orange}{C \epsilon^2 \cdot T}\]

to the estimate obtained after applying the Gronwall inequality.

The multiplicative terms contribute

\[ \int_0^T \textcolor{blue}{\norm{U_H^\epsilon}_{H^3}} + \textcolor{blue}{\norm{u_H}_{H^3}} + \norm{U_H^\epsilon}_{H^3}^2 + C \norm{u_H}_{H^3}^2 + \norm{c}_{H^2}^2 + \textcolor{orange}{C} \diff \tau,\]

which can be estimated with

\begin{equation}
  \begin{split}
    & \int_0^T \norm{U_H^\epsilon}_{H^3} + \norm{u_H}_{H^3} \diff \tau + C + \textcolor{orange}{C \cdot T} \\
    & \leq \int_0^T \norm{1 \cdot U_H^\epsilon}_{H^3} + \norm{1 \cdot u_H}_{H^3} \diff \tau + C + \textcolor{orange}{C \cdot T}\\
    & \leq T^{1/2} \bigg (\int_0^T \norm{U_H^\epsilon}_{H^3}^2 \bigg )^{1/2} \diff \tau + T^{1/2} \bigg (\int_0^T \norm{u_H}_{H^3}^2 \bigg )^{1/2} \diff \tau + C + \textcolor{orange}{C \cdot T}\\
    & \leq \sqrt{T} C + C + \textcolor{orange}{ C \cdot T}.
  \end{split}
\end{equation}

The issue here is that there is no known available estimate for $L^1$ estimate in time for the Navier-Stokes equations for infinite time interval, only for the case of a bounded domain, where $L^2$ boundedness implies $L^1$ boundedness directly. Using $\norm{U_H^\epsilon}_{H^3} \leq 1 + \norm{U_H^\epsilon}_{H^3}^2$ does not help in terms of obtaining a uniform estimate, as it leads to $\int_0^T 1 + \norm{U_H^\epsilon}_{H^3}^2 \diff \tau,$ which contains a $T.$

Thus the main difference from Section 5 in Li-Titi is that while in their case they had an $\epsilon^2 C$ bound where $C=C(L_1, L_2, \norm{v_0}_{H_1}),$ \textbf{we have a $C$ that depends also on $T,$} due to the source term as well.
...
\begin{equation}
  \begin{split}
    ...
    & \leq (\epsilon C + \textcolor{orange}{C \epsilon^2 \cdot T}) \exp{(C \sqrt{T} + C + \textcolor{orange}{C \cdot T})}.
  \end{split}
\end{equation}

By this, it seems that not just the convergence rate weakens from $\epsilon^2$ to $\epsilon$, but also the uniformity in time is lost. We can not use $T=\infty$ in this $C^\epsilon$ bound itself, but the estimate holds for any finite point in time --- the convergence gets slower as time evolves.

Assuming that the global existence for $c$ works, then this above estimate suggests that $c^\epsilon$ exists globally, too. Otherwise, if there is a finite maximal $T^\epsilon_*$ existence time, then the norm of the $c^\epsilon - c$ difference at $T^\epsilon_*$ is bounded by

\begin{equation}
  \begin{split}
  & \norm{C^\epsilon}_{L^\infty(0,T^\epsilon_*; H^1(\Omega))} + \norm{\nabla_d C^\epsilon}_{L^2(0,T^\epsilon_*; H^1(\Omega))} \\
  & \leq (\epsilon C + \textcolor{orange}{C \epsilon^2 \cdot T^\epsilon_*}) \exp{(C \sqrt{T^\epsilon_*} + C + \textcolor{orange}{C \cdot T^\epsilon_*})} < \infty,
  \end{split}
\end{equation}

using that $T^\epsilon_* < \infty,$ and then because of the finite energy it could be continued after $T^\epsilon_*,$ which is a contradiction.

In other words, the bound does not blow up at any finite time, only at infinity.

\textbf{To summarise},
\begin{enumerate}
  \item it seems that both $c^\epsilon$ and $c$ exist globally,
  \item for any given fix point in time $T$ we have that $c^\epsilon \to c$ in $L^\infty H^1$ and $\nabla_d c^\epsilon \to \nabla_d c$ in $L^2 H^1$ with a rate of $\epsilon,$
  \item considering a fix $\epsilon$ value and the corresponding $c^\epsilon$ anisotropic solution in time, the $c^\epsilon$ and $c$ get further away from each other in norm as time passes by.
\end{enumerate}

\section{Explanation of the domain structure}
...
\textbf{Since the zero Dirichlet boundary conditions are not sufficient in this case, but we also need the derivatives of functions to be zero on the boundary, we opt for using the more simple and convenient / transparent PBC solution for the boundary conditions }
