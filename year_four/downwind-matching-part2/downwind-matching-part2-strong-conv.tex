...
Assuming that the global existence for $c$ works, then this above estimate suggests that $c^\epsilon$ exists globally, too. Otherwise, if there is a finite $T^\epsilon_*$ existence time, then the norm of the $c^\epsilon - c$ difference at $T^\epsilon_*$ is bounded by

\[ \norm{C^\epsilon}_{L^\infty(0,T^\epsilon_*; H^1(\Omega))} \leq \epsilon C \exp{(C \sqrt{T^\epsilon_*} + C)} < \infty,\]

using that $T^\epsilon_* < \infty.$
...
\begin{enumerate}
  ...
  \item considering a fix $\epsilon$ value and the corresponding $c^\epsilon$ anisotropic solution, $c^\epsilon$ and $c$ get further away from each other in norm as time passes by.
\end{enumerate}
