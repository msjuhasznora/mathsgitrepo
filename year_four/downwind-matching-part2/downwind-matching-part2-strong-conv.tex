...
\begin{equation}
  \begin{split}
    ...
    & \leq C \| \partial_1 U^\epsilon \|_2^{1/2} \| \nabla \partial_1 U^\epsilon \|_2^{1/2} \norm{c}_{H^2} \norm{\partial_1 C^\epsilon}_2 + C \| U^\epsilon \|_{H^2} \norm{c}_{H^2} \norm{\partial_1 C^\epsilon}_2 \\
...
As we will show later, $\norm{c}_{H^2}$ is finite for the case of an $H^3$-regular initial horizontal velocity ${u_h}_0,$ which we assumed.

\textbf{Step 3: estimate for $I_3(i), i=1,2,3.$}

Due to their structure, computations for $\int_\Omega u \nabla C^\epsilon \partial_{ii} C^\epsilon$ are analogous to those seen in \textbf{Step 1.}
