...
Using $(V^\epsilon, W^\epsilon, C^\epsilon)(t_0) = 0,$ applying the Gronwall inequality we \todo{we have a new $\int_0^t C \diff s$ part, which has to be bounded} have 
...
\begin{equation} \label{result_theorem1}
  \begin{split}
    ...
    & \times \exp{ \bigg (\int_0^t \textcolor{blue}{C}+ (1+ \epsilon^4) \norm{\Delta v}^2_2 \norm{\nabla \Delta v}^2_2 + \textcolor{green}{\norm{\nabla c}^2_2 \norm{\Delta c}^2_2} \diff s \bigg ) }.
  \end{split}
\end{equation}

\end{proof}

Problem \todo{problem}: the $\int_0^t \textcolor{blue}{C} \diff s$ new term is different. For the other terms appearing on the right-hand side of the estimate, we know a bound of the form $\int_0^\infty \norm{\nabla v} \diff s \leq C.$ On this new standalone $\int_0^t \textcolor{blue}{C} \diff s$ term we don't have something like that. These new terms come from the i), ii), and iv) points of estimating the Coriolis terms.
