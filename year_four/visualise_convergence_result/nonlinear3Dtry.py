...
u1, u2, u3 = split(u)
v1, v2, v3 = split(v)
F = c1 * inner(u, grad(u1)) * v1 * dx + c2 * inner(u, grad(u2)) * v2 * dx + c3 * c3 * inner(u, grad(u3)) * v3 * dx + \
inner(grad(u1),grad(v1)) * dx + inner(grad(u2),grad(v2)) * dx + c3 * c3 * inner(grad(u3),grad(v3)) * dx + \
- inner(theta, v) * ds(1)
