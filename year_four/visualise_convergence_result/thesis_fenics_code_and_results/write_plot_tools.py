...
def plot_error_values(resultsfolder, problem_data):
    ...
    if problem_data.id == 4:
        ...
        plt.title("Testcase " + str(problem_data.id), fontsize = 16)
        plt.ylim(top = 0)
        plt.ylim(bottom = -16)
        plt.xlabel("The nx parameter in mesh(nx, nx)", fontsize = 16)
        plt.ylabel("log(L2 error)", fontsize = 16)
        plt.xticks(fontsize = 14)
        plt.yticks(fontsize = 14)
        plt.gcf().subplots_adjust(bottom=0.15)
        plt.gcf().subplots_adjust(left=0.2)
        plt.gca().yaxis.set_major_formatter(StrMethodFormatter('{x:,.1f}'))
        plt.savefig(resultsfolder + "log_errorvalues" + str(problem_data.id) + "/L_2_errors" + str(problem_data.id) + ".pdf")
        plt.clf()
        ...
        plt.ylim(bottom = -10)
        plt.xlabel("The nx parameter in mesh(nx, nx)", fontsize = 16)
        plt.ylabel("log(H1 error)", fontsize = 16)
        plt.xticks(fontsize = 14)
        plt.yticks(fontsize = 14)
        plt.gcf().subplots_adjust(bottom=0.15)
        plt.gcf().subplots_adjust(left=0.2)
        plt.gca().yaxis.set_major_formatter(StrMethodFormatter('{x:,.1f}'))
        plt.savefig(resultsfolder + "log_errorvalues" + str(problem_data.id) + "/H_1_errors" + str(problem_data.id) + ".pdf")
        plt.clf()
        ...
        plt.legend([line1, line2, line3, line4], ['horizontal velocity', 'vertical velocity', 'pressure', 'concentration'])
        plt.title("EOC values for testcase " + str(problem_data.id), fontsize = 16)
        plt.ylim(top = 4)
        plt.ylim(bottom = 1)
        plt.xlabel("The nx parameter in mesh(nx, nx)", fontsize = 16)
        plt.ylabel("EOC", fontsize = 16)
        plt.xticks(fontsize = 14)
        plt.yticks(fontsize = 14)
        plt.gcf().subplots_adjust(bottom=0.15)
        plt.gcf().subplots_adjust(left=0.2)
