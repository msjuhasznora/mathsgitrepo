...
### idea to fix the disappearing dependence of the first step of u3. we could split the method to further steps,
...
while eps > DOLFIN_EPS:
    ...
    F1_anisotropic = (1/k)*( (u1 - u_prev1)*v1 + (u2 - u_prev2)*v2 + eps*eps*(u3 - u_prev3)*v3 ) * dx + \
        inner(u, grad(u1)) * v1 * dx + inner(u, grad(u2)) * v2 * dx + \
        eps*eps*inner(u, grad(u3)) * v3 * dx + \
        - alpha * u2 * v1 * dx + alpha * u1 * v2 * dx + \
        inner(grad(u1),grad(v1)) * dx + inner(grad(u2),grad(v2)) * dx + eps*eps*inner(grad(u3),grad(v3)) * dx + \
        eps * beta * u3 * v1 * dx - eps * beta * u1 * v3 * dx + \
        - inner(f, v) * dx + \
        - inner(theta, v) * ds(1)
