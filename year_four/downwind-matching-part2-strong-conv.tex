...
\begin{itemize}
  ...
  \item We need the pollution concentration-including version of Corollary 3.1, namely, the estimates for the term $\norm{\nabla c}^2_2 \norm{\Delta c}^2_2$ in the integral in the above result. Corollary 3.1 gives a constant estimate only for the velocities, e.g. $\norm{\nabla \Delta v}^2_2.$
  \item We need the $L^2$ estimate (analogous to Proposition 5.1.) to $C^\epsilon,$ while we already know the result for $V^\epsilon, \epsilon W^\epsilon.$
  \item Analogous existence result for (E-SNS).
  \item Local well-posedness of (E-SNS).

\end{itemize}

When these missing pieces are clear, then from the $L^2$ estimate we have
...
$T^*_\epsilon$ being the maximal existence time.
...
Using Corollary 3.1 and an analogous result bounding the $c$ norms too, from (\ref{result_theorem1}) we get
...
for any $\epsilon \in (0, \epsilon_0).$ From which we have
...
From this we have $T^*_\epsilon = \infty.$
