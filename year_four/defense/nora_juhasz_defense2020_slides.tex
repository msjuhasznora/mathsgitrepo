...
\begin{enumerate}
\item \textbf{Framework 1.} Geophysical coordinate system + weak solutions. \textit{\textcolor{terracotta}{Key focus}:} Strong convergence of the concentration function, visualisation of the results.
\item \textbf{Framework 2.} Downwind - matching coordinate system + weak solutions. \textit{\textcolor{terracotta}{Key focus}:} Classical, non-periodic domain, Coriolis terms.
\item \textbf{Framework 3.} Downwind - matching coordinate system + strong solutions. \textit{\textcolor{terracotta}{Key focus}:} Strong convergence of the concentration function, existence result for the limit system.
\end{enumerate}
\end{enumerate}
...
We extend the velocity system: \textcolor{terracotta}{pollutant present in the atmosphere} in concentration $p_c.$
...
\item \textcolor{terracotta}{$u^\epsilon_3 c^\epsilon$} --- \textbf{?}
...
\begin{frame}
Previous observations $\to$ \textcolor{terracotta}{avoid linearisation techniques} + conserve the nonlinear nature of the problem. We focus our attention to a simplified scenario --- key: keep the main challenging feature yet make the problem manageable.
...
\textbf{Boundary conditions:} weak solutions framework $\to$ we aim to be as close as possible to the original domain --- \textcolor{terracotta}{classical non-periodic local domain} on the surface of the Earth + minimal modifications due to the new $\epsilon$ term (inland domains).
...
\begin{frame}
We use this theorem to verify the strong convergence of $u_3^\epsilon,$ \textcolor{terracotta}{keeping in mind that}
\begin{description}
...
\begin{theorem}
Let ${\mathbf{u}_h}_0 \in H^3(\Omega),$ ${u_3}_0 \in H^2(\Omega)$ with $\nabla \cdot \mathbf{u}_0 = 0$, let $c_0 \in H^2 (\Omega),$ and let's assume that the given boundary conditions hold. Then there exists a unique global strong solution $(\mathbf{u}, \textcolor{terracotta}{c})$ of the downwind-matching hydrostatic polluted atmosphere limit model:
%
    \begin{equation} \nonumber
...
\begin{frame}
The proof is partly based on based on [C. Cao, J. Li, and E. S. Titi, 2014] --- a system that is very similar to ours (including an incomplete Laplacian).

\textit{Similarities}: strong analogy between the equations describing the dynamics of $T$ and $c$.

\textcolor{terracotta}{Cardinal difference}: Reciprocal vs. one-sided coupling $\to$ overall this contributes to a very significant structural divergence.
...
\changefontsizes{11pt}
We only need to work on the \textcolor{terracotta}{continuability of $c$}: multiplying the governing equation by $\Delta^2 c,$ and integrating the result on $\Omega$ gives
\changefontsizes{10pt}
\begin{equation} \nonumber
\begin{split}
