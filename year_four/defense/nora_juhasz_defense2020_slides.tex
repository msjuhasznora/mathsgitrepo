...
\end{equation}
%
\textit{\textbf{Step two.}} We define a new concentration function $c^*$: $c^* = c + \frac{z}{a}.$ Result: we obtain a function that is zero on both the upper and lower boundary of the domain. $to$ We can consider the standard extensions of $\mathbf{u}_h$, $u_3$, $p$ and $c$ to the new $\Omega = M \times (-a, a)$ domain.
...
Combining this convergence result with the strong converges already detailed for the velocity, our proof is finished.
\end{frame}
...
Existence of the unique local strong solution $\mathbf{u}_h, c$: as the strong limit of the sequences $\mathbf{u}_h^\mu, c^\mu$ as $\mu \to 0$.
