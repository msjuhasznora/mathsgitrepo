...
\begin{equation} \nonumber
  \begin{split}
    ...
    & + \int_{Q_t}  ( \gamma u_2^\epsilon u_1 - \epsilon \beta u_3^\epsilon u_1 + \textcolor{jasper}{\epsilon} \alpha u_3^\epsilon u_2 - \gamma u_1^\epsilon u_2 - \textcolor{jasper}{\epsilon} \alpha u^\epsilon_2 u_3 + \textcolor{jasper}{\epsilon} \beta u_1^\epsilon u_3)
  \end{split}
\end{equation}
%
...
Let ${\mathbf{u}_h}_0 \in H^3(\Omega),$ ${u_3}_0 \in H^2(\Omega),$ $\int_\Omega {\mathbf{u}_h}_0 \diff x \diff y \diff z = 0,$ $\nabla \cdot \mathbf{u}_0 = 0,$ and let $c_0 \in H^2 (\Omega).$ There is an $\epsilon_0 > 0$ such that for any $\epsilon < \epsilon_0,$ the strong solution $(\mathbf{u}^\epsilon, \textcolor{mediumred-violet}{c^\epsilon})$ of the anisotropic equations exists globally in time and as $\epsilon \to 0$, it converges strongly to $(\mathbf{u}, \textcolor{mediumred-violet}{c}).$ 
...
\end{equation}
We introduce $$C^\epsilon = c^\epsilon-c \text{ and } S^\epsilon = s^\epsilon-s.$$
