...
\author[N\'ora Juh\'asz]{\textbf{PhD thesis defense} \\[2mm] Candidate: N\'ora Juh\'asz \\[1mm] \textit{Supervisor: Prof. Donatella Donatelli}}
...
$\textbf{Additional scaling equation}$: it is not derived from the shallowness of the domain --- it is suggested by the fact that in the downwind direction it is natural to assume that $\textit{ the diffusion is negligible compared to downwind transport}$ i.e.

    $$\abs{u_1 \partial_1 c} \gg \abs{ K_x \partial_{11}^2 c } ,$$
    which leads us to using
    $$K_x = \textcolor{jasper}{\epsilon} K_1.$$
    
The concentration equation becomes
$$ \partial_t c^\epsilon + \mathbf{u^\epsilon} \cdot \nabla c^\epsilon = \textcolor{jasper}{\epsilon} K_1 \partial_{11} ^ 2 c^\epsilon + K_2 \partial_{22} ^ 2 c^\epsilon + K_3 \partial_{33} ^ 2 c^\epsilon + s_\epsilon $$
...
\begin{equation} \nonumber
  \partial_t u^\epsilon _1 + \mathbf{u}^\epsilon \cdot \nabla u^\epsilon _1 -\Delta_\nu u^\epsilon _1 -\gamma u_2^\epsilon + \textcolor{jasper}{\epsilon} \beta u_3^\epsilon + \partial_1 p^\epsilon = 0 \text{ in } \Omega \times (0,T)
\end{equation}

\begin{equation} \nonumber
  \partial_t u_2^\epsilon + \mathbf{u}^\epsilon \cdot \nabla u_2^\epsilon -\Delta_\nu u_2^\epsilon + \gamma u_1^\epsilon - \textcolor{jasper}{\epsilon} \alpha u^\epsilon_3  + \partial_2 p^\epsilon = 0 \text{ in } \Omega \times (0,T)
\end{equation}

\begin{equation} \nonumber
  \textcolor{jasper}{\epsilon^2} \{  \partial_t u_3^\epsilon + \mathbf{u}^\epsilon \cdot \nabla u_3^\epsilon -\Delta_\nu u_3^{\epsilon} \} + \textcolor{jasper}{\epsilon} \alpha u^\epsilon_2 - \textcolor{jasper}{\epsilon} \beta u_1^\epsilon + \partial_3 p^\epsilon = 0 \text{ in } \Omega \times (0,T)
\end{equation}
...
\begin{equation} \nonumber
   \partial_t c^\epsilon + \mathbf{u^\epsilon} \cdot \nabla c^\epsilon = \textcolor{jasper}{\epsilon} K_1 \partial_{11} ^ 2 c^\epsilon + K_2 \partial_{22} ^ 2 c^\epsilon + K_3 \partial_{33} ^ 2 c^\epsilon + s^\epsilon \text{ in } \Omega \times (0,T).
\end{equation}
...
\textbf{Boundary conditions:} weak solutions framework $\to$ we aim to be as close as possible to the original domain --- \textcolor{mediumred-violet}{classical non-periodic local domain} on the surface of the Earth + minimal modifications due to the new $\textcolor{jasper}{\epsilon}$ term (inland domains).
...
The pair $(\mathbf{u}^\epsilon, c^\epsilon)$ is a weak solution of the anisotropic system if
$$\mathbf{u}^\epsilon = (\mathbf{u}^\epsilon_h, u^\epsilon_3) \in L^2 (0,T ; \mathbf{V}) \cap C_w (0, T; L^2 (\Omega)^3)$$
and
$$c^\epsilon \in L^\infty (0,T; L^2(\Omega)) \cap L^2 (0,T; H^1_{\Gamma_A}(\Omega)),$$ and the following integral identities hold:
%
\begin{equation} \nonumber
\begin{split}
& \int_0^T \bigg [ -(\mathbf{u}^\epsilon_h, \partial_t \mathbf{\tilde{u}}_h) + (\nabla_\nu \mathbf{u}^\epsilon_h, \nabla_\nu \mathbf{\tilde{u}}_h) - (\mathbf{u}^\epsilon_h, (\mathbf{u}^\epsilon \cdot \nabla) \mathbf{\tilde{u}}_h) + (b(\mathbf{u}^\epsilon_h), \mathbf{\tilde{u}}_h) \bigg ] \diff t \\
& + \textcolor{jasper}{\epsilon} \int_0 ^ T \big [ (\beta u_3 ^\epsilon, \tilde{u}_1) - (\beta u_1^\epsilon, \tilde{u}_3) \big ] \diff t + \textcolor{jasper}{\epsilon} \int_0 ^ T \big [ (\alpha u_2 ^\epsilon, \tilde{u}_3) - (\alpha u_3^\epsilon, \tilde{u}_2) \big ] \diff t\\
& + \textcolor{jasper}{\epsilon^2} \int _0 ^ T \big [ -(u_3^\epsilon, \partial_t \tilde{u}_3) +(\mathbf{u}^\epsilon \cdot \nabla u_3 ^ \epsilon, \tilde{u}_3) + (\nabla_\nu u_3^\epsilon, \nabla_\nu \tilde{u}_3) \big ] \diff t \\
& \hspace{0.3cm} = - ({\mathbf{u}^\epsilon_h}_0, \mathbf{\tilde{u}}_h(0)) - \textcolor{jasper}{\epsilon^2} ({u^\epsilon_3}_0, \tilde{u}_3 (0))
\end{split}
\end{equation}
...
\begin{equation}
\begin{split} \nonumber
& \int_0^T \bigg [ -(c^\epsilon, \partial_t \tilde{c}) -(\mathbf{u}^\epsilon c^\epsilon, \nabla \tilde{c}) + \textcolor{jasper}{\epsilon} K_1 (\partial_1 c^\epsilon, \partial_1 \tilde{c}) \\
& + K_2 (\partial_2 c^\epsilon, \partial_2 \tilde{c}) + K_3 (\partial_3 c^\epsilon, \partial_3 \tilde{c}) \bigg ] \diff t \\
& =  - (c_0, \tilde{c} (0)) + \int_0^T (s^\epsilon,\tilde{c}) \diff t
\end{split}
\end{equation}
%
for all $$\mathbf{\tilde{u}} = (\mathbf{\tilde{u}}_h, \tilde{u}_3) \in H^1 (0,T;\mathbf{V}) \text{ with } \mathbf{\tilde{u}}(T) = 0$$
and
$$ \tilde{c} \in H^1 (0,T, H^3_{\Gamma_A} (\Omega)), \text{ with } \tilde{c} (T) = 0.$$
...
The pair $(\mathbf{u}, c)$ is a weak solution of the a hydrostatic system if
$$\mathbf{u} = (\mathbf{u}_h, u_3) \in L^2 (0, T; \mathbf{W}) \cap L^\infty (0,T; L^2(\Omega)^3), \mathbf{u}_h \in L^\infty(0,T; H^1(\Omega)),$$
and
$$c \in L^\infty (0, T; L^2_{\Gamma_A}(\Omega)) \text{ with } \partial_2 c, \partial_3 c \in L^2(0, T; L^2(\Omega)),$$
and the following integral identities hold:
%
\begin{equation}\nonumber
\begin{split}
& \int_0^T \bigg[ -(\mathbf{u}_h, \partial_t \mathbf{\tilde{u}}_h) + (\nabla_\nu \mathbf{u}_h, \nabla_\nu \mathbf{\tilde{u}}_h) - (\mathbf{u}_h, (\mathbf{u} \cdot \nabla) \mathbf{\tilde{u}}_h) + (b(\mathbf{u}_h), \mathbf{\tilde{u}}_h) \bigg ] \diff t \\
& = - ({\mathbf{u}_h}_0, \mathbf{\tilde{u}}_h(0))
\end{split}
\end{equation}
%
\end{frame}
...
%
for all
$$\mathbf{\tilde{u}} = (\mathbf{\tilde{u}}_h, \tilde{u}_3) \in H^1 (0,T, \mathbf{W}), \text{ with } \mathbf{\tilde{u}}_h (T) = 0$$
and 
$$\tilde{c} \in H^1 (0,T, H^2_{\Gamma_A} (\Omega)), \tilde{c} (T) = 0.$$
\end{frame}
...
\item Convergence results obtained from the energy inequality:
    ...
    &  \textcolor{jasper}{\epsilon^2} u_3^\epsilon \to 0 &\quad & \text{strongly in $L_t^\infty L_x^2 \cap L_t^2 H_x^1$,} \nonumber \\ 
    ...
    & \textcolor{jasper}{\sqrt{\epsilon}} \partial_1 c^\epsilon \rightharpoonup e &\quad & \text{weakly for some limit element $e$ in $L_t^2 L_x^2$.} \nonumber
  \end{align}
...
By using the strong solution $(\mathbf{u}_h,u_3)$ as a test function in the weak formulation for the velocity of the anisotropic system, we have the following integral identity:
%
\begin{equation} \nonumber
  \begin{split}
    & - \frac{\textcolor{jasper}{\epsilon^2}}{2} \norm{u_3(t)}_2^2 + \bigg ( (\mathbf{u}_h^\epsilon \cdot \mathbf{u}_h + \textcolor{jasper}{\epsilon^2} u_3^\epsilon u_3) \diff x \diff y \diff z  \bigg ) (t) \\
    & + \int_{Q_t} (-\mathbf{u}_h^\epsilon \cdot \partial_t \mathbf{u}_h + \nabla \mathbf{u}_h^\epsilon : \nabla \mathbf{u}_h + \textcolor{jasper}{\epsilon^2} \nabla u_3^\epsilon \cdot \nabla u_3) \diff x \diff y \diff z \diff s \\
    & = \frac{\textcolor{jasper}{\epsilon^2}}{2} \norm{{u_3}_0}_2^2 + \norm{{\mathbf{u}_h}_0}_2^2 + \textcolor{jasper}{\epsilon^2} \int_{Q_t} \bigg ( \int_0^z \partial_t \mathbf{u}_h \diff z' \bigg ) \cdot \nabla_h U_3^\epsilon \diff x \diff y \diff z \diff s \\
    & - \int_{Q_t} [(\mathbf{u}^\epsilon \cdot \nabla) \mathbf{u}_h^\epsilon \cdot \mathbf{u}_h + \textcolor{jasper}{\epsilon^2} \mathbf{u}^\epsilon \cdot \nabla u_3^\epsilon u_3] \diff x \diff y \diff z \diff s \\
    & + \int_{Q_t}  ( \gamma u_2^\epsilon u_1 - \textcolor{jasper}{\epsilon} \beta u_3^\epsilon u_1 + \textcolor{jasper}{\epsilon} \alpha u_3^\epsilon u_2 - \gamma u_1^\epsilon u_2 - \textcolor{jasper}{\epsilon} \alpha u^\epsilon_2 u_3 + \textcolor{jasper}{\epsilon} \beta u_1^\epsilon u_3)
  \end{split}
\end{equation}
%
...
... but test functions have compact support in time.
\begin{description}
...
\begin{equation} \nonumber
\begin{split}
& \frac{1}{2}( \norm{\mathbf{u}_h^\epsilon (t_0)}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{u_3^\epsilon(t_0)}_2^2)+ \int_0^{t_0} (\norm{\nabla \mathbf{u}_h^\epsilon}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{\nabla u_3^\epsilon}_2^2) \diff s \\
& \leq \frac{1}{2} (\norm{{\mathbf{u}_h}_0}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{{u_3}_0}_2^2).
\end{split}
\end{equation}
\end{itemize}
\end{frame}
...
\begin{frame}{Proof - Step 2: Main inequality}
  ...
  \begin{equation} \nonumber
    \begin{split}
      & \frac{1}{2} (\norm{\mathbf{U}_h^\epsilon}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{U_3^\epsilon}_2^2) (t_0) + \int_0^{t_0} (\norm{\nabla \mathbf{U}_h^\epsilon}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{\nabla U_3^\epsilon}_2^2) \diff t  \\
      & \leq - \textcolor{jasper}{\epsilon^2} \int_{Q_{t_0}} \bigg [ \bigg ( \int_0^z \partial_t \mathbf{u}_h \diff z' \bigg ) \cdot \nabla_h U_3^\epsilon + \nabla u_3 \cdot \nabla U_3^\epsilon \bigg ] \diff x \diff y \diff z \diff t \\
      ...
      & + \textcolor{jasper}{\epsilon^2} \int_{Q_{t_0}} \mathbf{u}^\epsilon \cdot \nabla u_3^\epsilon u_3 \diff x \diff y \diff z \diff t \\
      & + \int_{Q_{t_0}}  \textcolor{jasper}{\epsilon} \beta u_3^\epsilon u_1 - \textcolor{jasper}{\epsilon} \alpha u_3^\epsilon u_2 + \textcolor{jasper}{\epsilon} \alpha u_2^\epsilon u_3 - \textcolor{jasper}{\epsilon} \beta u_1^\epsilon u_3 \diff x \diff y \diff z \diff t \\
      & := I_1 + I_2 + I_3 + I_4
    \end{split}
  \end{equation}
...
\item $\kappa \textcolor{jasper}{\epsilon^2}$ - type terms,
...
We add $ - \textcolor{jasper}{\epsilon} \beta u_3 u_1 +  \textcolor{jasper}{\epsilon} \beta u_3 u_1 - \textcolor{jasper}{\epsilon} \alpha u_3 u_2 + \textcolor{jasper}{\epsilon} \alpha u_3 u_2 = 0 $ to $I_4:$ 

\begin{equation}\nonumber
\begin{split}
& I_4 = \int_{Q_{t_0}}  ( \textcolor{jasper}{\epsilon} \alpha U_2^\epsilon u_3 - \textcolor{jasper}{\epsilon} \beta U_1^\epsilon u_3 + \textcolor{jasper}{\epsilon} \beta U_3^\epsilon u_1 - \textcolor{jasper}{\epsilon} \alpha U_3^\epsilon u_2 ) \diff x \diff y \diff z \diff t \\
...
\begin{equation}\nonumber
\begin{split}
I_{41} = & \int_{Q_{t_0}} \textcolor{jasper}{\epsilon} \alpha U_2^\epsilon u_3 \diff x \diff y \diff z \diff t \lesssim \int_0^{t_0} \norm{U_2^\epsilon} \norm{\textcolor{jasper}{\epsilon} u_3} \diff t \\
& \lesssim \int_0^{t_0} \zeta \norm{U_2^\epsilon}^2_2 \diff t + \int_0^{t_0} \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \norm{u_3}^2_2 \diff t \\
& \lesssim \int_0^{t_0} \zeta \norm{\nabla U_2^\epsilon}^2_2 \diff t + \int_0^{t_0} \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \norm{u_3}^2_2 \diff t,
\end{split}
\end{equation}
      
$$\int_0^{t_0} \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \norm{u_3}^2_2 \leq \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \norm{u_3}^2_{L^2(Q_{t_0})} \leq \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \kappa (\norm{{\mathbf{u}_h}_0}_{H^1}, \Gamma_G).$$

\end{frame}

\begin{frame}
For the term $ I_{43} = \textcolor{jasper}{\epsilon} \beta U_3^\epsilon u_1$ we use the idea that $\| U_3^\epsilon \| \leq \| \nabla \mathbf{U}_h^\epsilon \|.$ As we have $\partial_3 u_3^\epsilon = \nabla_h \mathbf{u}_h^\epsilon,$ and $\partial_3 u_3 = \nabla_h \mathbf{u}_h,$ we naturally have $\partial_3 U_3^\epsilon = \nabla_h \mathbf{U}_h^\epsilon,$ and by the vertical Poincar\'e inequality we have $$\norm{U_3^\epsilon} \leq \norm{\partial_3 U_3^\epsilon} = \norm{\nabla \mathbf{U}_h^\epsilon}.$$ We have
      
      $$ \hspace{-1em}\int_{Q_{t_0}} \textcolor{jasper}{\epsilon} \beta U_3^\epsilon u_1 \diff x \diff y \diff z \diff t \lesssim \int_0^{t_0} \textcolor{jasper}{\epsilon} \norm{U_3^\epsilon}_2 \norm{u_1}_2 \diff t \lesssim \int_0^{t_0} \textcolor{jasper}{\epsilon} \| \nabla \mathbf{U}_h^\epsilon \|_2 \norm{u_1}_2 \diff t, $$
      
      and finally, after applying the Young inequality we arrive to
      
      \begin{equation}\nonumber
        \begin{split}
          I_{43} & \lesssim \zeta \int_0^{t_0} \norm{\nabla \mathbf{U}_h^\epsilon}_2^2 \diff s+ \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \int_0^{t_0} \norm{u_1}_2^2 \diff s \\
          & \lesssim \zeta \int_0^{t_0} \norm{\nabla \mathbf{U}_h^\epsilon}_2^2 \diff t + \frac{1}{\zeta} \textcolor{jasper}{\epsilon^2} \kappa (\norm{{\mathbf{u}_h}_0}_{H^1}, \Gamma_G).
        \end{split}
      \end{equation}
\end{frame}
...
\begin{equation}
  \begin{split} \nonumber
    ...
    & \leq \textcolor{jasper}{\epsilon^2} \kappa (\norm{{\mathbf{u}_h}_0}_{H^1}, \Gamma_G) (\norm{{\mathbf{u}_h}_0}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{{u_3}_0}_2^2 + 1)^2,
  \end{split}
\end{equation}
%
Thus, we now have
$$ (\norm{\mathbf{U}_h^\epsilon}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{U_3^\epsilon}_2^2)(t) + \int_0^t (\norm{\nabla \mathbf{U}_h^\epsilon}_2^2 + \textcolor{jasper}{\epsilon^2} \norm{\nabla U_3^\epsilon}_2^2) \diff s \leq \textcolor{jasper}{\epsilon^2} \kappa.$$
\end{frame}
...
%
\begin{equation} \nonumber
\partial_t c^\epsilon + \mathbf{u}_h^\epsilon \cdot \nabla_h c^\epsilon + u_3^\epsilon \bigg ( \partial_3 c^\epsilon - \frac{1}{a} \bigg )= \textcolor{jasper}{\epsilon} K_1 \partial_{11} ^ 2 c^\epsilon + \Delta_d c^\epsilon + s^\epsilon \text{ in } \Omega \times (0,T).
\end{equation}
%
...
\begin{frame}
\textit{\textbf{Step three.}} Now the anisotropic and hydrostatic systems are set for
%
\begin{equation} \nonumber
\begin{split}
...
Let ${\mathbf{u}_h}_0 \in H^3(\Omega),$ ${u_3}_0 \in H^2(\Omega),$ $\int_\Omega {\mathbf{u}_h}_0 \diff x \diff y \diff z = 0,$ $\nabla \cdot \mathbf{u}_0 = 0,$ and let $c_0 \in H^2 (\Omega).$ There is an $\epsilon_0 > 0$ such that for any $\textcolor{jasper}{\epsilon} < \epsilon_0,$ the strong solution $(\mathbf{u}^\epsilon, \textcolor{mediumred-violet}{c^\epsilon})$ of the anisotropic equations exists globally in time and as $\textcolor{jasper}{\epsilon} \to 0$, it converges strongly to $(\mathbf{u}, \textcolor{mediumred-violet}{c}).$ 
In more detail, the following strong convergence results hold:
\begin{align} 
& (\mathbf{u}_h^\epsilon, \textcolor{jasper}{\epsilon} u_3^\epsilon) \to (\mathbf{u}_h, 0) &\quad & \text{in $L_t^\infty H_x^2$,} \nonumber \\
& (\nabla \mathbf{u}_h^\epsilon, \textcolor{jasper}{\epsilon} \nabla u_3^\epsilon, u_3^\epsilon) \to (\nabla \mathbf{u}_h, 0, u_3) &\quad & \text{in $L_t^2 H_x^2$,} \nonumber \\ 
...
This part is based on [Li, J., Titi, E. S. (2019)], and $\textcolor{jasper}{\epsilon} < \epsilon_0.$

For an ${\mathbf{u}_h}_0 \in H^3(\Omega)$ initial data the velocity difference functions satisfy
%
\begin{equation} \nonumber
\sup_{0 \leq t < \infty}{\norm{(\mathbf{U}_h^\epsilon, \textcolor{jasper}{\epsilon} U_3^\epsilon)}_{H^2}^2} + \int_0^\infty \norm{\nabla (\mathbf{U}_h^\epsilon, \textcolor{jasper}{\epsilon} U_3^\epsilon)}_{H^2}^2 \leq \kappa \textcolor{jasper}{\epsilon^2}.
\end{equation}
%
As a consequence, the following strong convergences hold for an ${\mathbf{u}_h}_0 \in H^3(\Omega)$ initial data:
\begin{align}
&  (\mathbf{u}_h^\epsilon, \textcolor{jasper}{\epsilon} u_3^\epsilon) \to (\mathbf{u}_h, 0) &\quad & \text{strongly in $L_t^\infty H_x^2$,} \nonumber \\
...
\end{equation}
We set $$C^\epsilon = c^\epsilon-c \text{ and } S^\epsilon = s^\epsilon-s.$$

The governing equation for $C^\epsilon$ then takes the form:

\begin{equation} \nonumber
  \begin{split}
   & \partial_t C^\epsilon + \mathbf{U}^\epsilon \nabla C^\epsilon  + \mathbf{U}^\epsilon \nabla c+ \mathbf{u} \nabla C^\epsilon \\
   & = \frac{1}{a} U_3^\epsilon + \textcolor{jasper}{\epsilon} K_1 \partial_{11} C^\epsilon + \textcolor{jasper}{\epsilon} K_1 \partial_{11} c + \Delta_d C^\epsilon + S^\epsilon.
   \end{split}
\end{equation}
\end{frame}
...
\begin{equation} \nonumber
  \begin{split}
    ...
    & + \norm{\mathbf{U}_h^\epsilon}_{H^3}^2 + \norm{\mathbf{U}_h^\epsilon}_{H^3}^2 \norm{\nabla c}_2^2 + \textcolor{jasper}{\epsilon} K_1 \norm{\partial_{11} c}_2^2 + \textcolor{jasper}{\epsilon^2} \\
...
%
\changefontsizes{10pt}
\[ \int_0^T B(t) \diff \tau = \kappa \int_0^T \textcolor{jasper}{\epsilon} \norm{\partial_{11} c}_2^2 + \norm{\mathbf{U}_h^\epsilon}_{H^3}^2 \norm{\nabla c}_2^2 + \norm{\mathbf{U}_h^\epsilon}_{H^3}^2 + \textcolor{jasper}{\epsilon^2} \diff \tau \lesssim \textcolor{jasper}{\epsilon} + \textcolor{jasper}{\epsilon^2} + \textcolor{jasper}{\epsilon^2} T,\]
\changefontsizes{12pt}
%
and
%
...
& \lesssim (\textcolor{jasper}{\epsilon} + \textcolor{jasper}{\epsilon^2} \cdot T) \exp{\kappa (\sqrt{T} + 1 + T)}.
\end{split}
\end{equation}
\end{frame}
...
\end{equation}
%
moreover the rate of convergence is $\textcolor{jasper}{\epsilon}$ (note however: the estimate's time-dependence also means that the convergence gets slower as time evolves).
