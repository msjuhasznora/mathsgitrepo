...
\textcolor{antiquefuchsia}{\textit{\textbf{Step one.}}} Let's consider $\Omega_0 = M \times (-a, 0)$ with $M = (0,1) \times (0,1)$ over an inland domain. We define the boundary conditions
%
\begin{equation}\nonumber
\begin{split}
...
\end{equation}
%
\textcolor{antiquefuchsia}{\textit{\textbf{Step two.}}} We define a new concentration function $c^*$: $c^* = c + \frac{z}{a}.$ Result: we obtain a function that is zero on both the upper and lower boundary of the domain. $to$ We can consider the standard extensions of $\mathbf{u}_h$, $u_3$, $p$ and $c$ to the new $\Omega = M \times (-a, a)$ domain.
...
\begin{frame}
\textcolor{antiquefuchsia}{\textit{\textbf{Step three.}}} Now the anisotropic and hydrostatic systems are combined with the
%
\begin{equation} \nonumber
\begin{split}
