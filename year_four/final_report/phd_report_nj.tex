...
Here we show a strong convergence result, and enable some technical changes regarding the boundary conditions: we use the minimal additional set of restrictions (the derivatives of the solutions are set to zero until degree 2) that allow us to benefit from a simpler structure that effectively simulates periodic domains, keeping a classical non-periodic domain however. Another important detail to highlight here is that in order to be able to bring the proof to a closure, we use an $H^3$ initial data for the horizontal velocity.
