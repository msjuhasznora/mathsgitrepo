...
If we assume $\mathbf{u}^\epsilon = O(1)$, then neglecting the $\epsilon^2$ and $\epsilon$ in the anisotropic equations, we formally arrive to the hydrostatic Navier-Stokes equations (i.e., primitive equations) combined with pollution.
    ...
    u_i (\cdot, t=0) = u_{0 i}, \text{ } c (\cdot, t=0) = c_0 \text{ in } \Omega, \text{ $i$ = 1,2}
    \end{equation}
...
In this subsection we write down the weak formulations of the two diffe\-rent systems in geophysical coordinates, and provide the main steps of the proof that shows that any weak solution of the anisotropic merged velocity -- pollution system converges to a weak solution of the hydrostatic system as $\epsilon$ goes to zero. In order to state the respective weak forms, we define the spaces these functions will be defined in.
...
\texttt{
\\
F = (1/dt)((u\_H - u\_prev\_H)*v\_H + $\epsilon^2$*(\textcolor{terracotta}{u3} - u\_prev3)*v3 ) + \\
inner(u\_prev, grad(u\_prev\_H)) * v\_H\\
 + $\epsilon^2$*inner(u\_prev, grad(u\_prev3)) * v3 + \\
 inner(grad(u\_H),grad(v\_H)) + $\epsilon^2$*inner(grad(\textcolor{terracotta}{u3}),grad(v3)) \\
}
...
\texttt{
\\
a = inner(\textcolor{blue}{u}, grad(u\_k\_H)) * v\_H + $\epsilon^2$*inner(u, grad(\textcolor{green}{u\_k3})) * v3 + \\
...
The new version of the anisotropic equations now takes the form below, where the three main changes are \textit{a)} the additional $\epsilon$ in the concentration equation, \textit{b)} the slightly stricter boundary conditions, and \textit{c)} the new Coriolis terms due to the coordinate system change:
...
Here we show a strong convergence result, and enable some technical changes regarding the boundary conditions: we use a small additional set of restrictions (the derivatives of the solutions are set to zero until degree 2) that allow us to benefit from a simpler structure that effectively simulates periodic domains in terms of lack of boundary integrals, keeping a classical non-periodic domain however. Another important detail to highlight here is that in order to be able to bring the proof to a closure, we use an $H^3$ initial data for the horizontal velocity.

The set of equations are identical as in the previous subsection, except for simplicity here we use $\epsilon-$ independent source terms in the anisotropic equations, i.e. $s^\epsilon = s \text{ } \forall \epsilon \geq 0.$
...
$$ \partial_t C^\epsilon + u^\epsilon \cdot \nabla c^\epsilon - u \cdot \nabla c = \epsilon K_1 \partial_{11} c^\epsilon + K_2 \partial_{22} C^\epsilon + K_3 \partial_{33} C^\epsilon,$$

which can be rewritten as

$$ \partial_t C^\epsilon + U^\epsilon \nabla C^\epsilon  + U^\epsilon \nabla c+ u \nabla C^\epsilon = \epsilon K_1 \partial_{11} C^\epsilon + \epsilon K_1 \partial_{11} c + K_2 \partial_{22} C^\epsilon + K_3 \partial_{33} C^\epsilon. $$

We take ${\mathbf{u}_h}_0 \in H^3(\Omega)$ (and as a consequence $\mathbf{u}_0 \in H^2(\Omega)$) and $\epsilon < \epsilon_0,$ where $\epsilon_0$ is the limit specified in \cite{li2019primitive}. We need the following important ingredients needed for estimating the difference function $C^\epsilon.$
...
\begin{theorem}
Let ${\mathbf{u}_h}_0 \in H^3(\Omega),$ with $\nabla \cdot \mathbf{u}_0 = 0$; $\epsilon < \epsilon_0,$ and $C_0 \in H^2;$ moreover $\mathbf{u}_0 = 0$ on $\partial \Omega$, $c_0 = 0$ on $\partial \Omega.$ Let $\nabla^k \mathbf{u} = 0 \text{ on } \Gamma, \nabla^k c = 0  \text{ on } \Gamma$ for $k= 0,1,2;$ and let the source terms $s^\epsilon = s \in L^2_t H^2_x$ for all $\epsilon \geq 0.$ Then the strong solution $(\mathbf{u}_h^\epsilon, u_3^\epsilon, c^\epsilon)$ of the anisotropic system (\ref{ANSC1-5}) converges to the strong solution $(\mathbf{u}_h, u_3, c)$ of the hydrostatic system (\ref{HNSC1-5}) in downwind-matching coordinates; formally, we have
$$(\mathbf{u}_h^\epsilon, u_3^\epsilon, c^\epsilon) \to (\mathbf{u}_h, u_3, c) \text{ in } L^\infty H^1.$$
\end{theorem}

Since the convergence result of the velocity part had already been proved, we focus on verifying $c^\epsilon \to c$ in $L^\infty H^1.$
...
We have been able to construct bounds for each term on the right-hand side using the H\"older, Ladyzhenskaya and Young inequalities. These eventually yield
...
where $i \neq j.$ Applying the Gr\"onwall inequality, we finally arrive to
...
\begin{itemize}
  \item "The polluted atmosphere as a shallow domain", Summer School on Reduced Order Methods in Computational Fluid Dynamics, SISSA, Trieste, July 8 - 12, 2019 (Poster presentation)
  \item "The polluted atmosphere as a shallow domain", The XVIII Italian Meeting on Hyperbolic Equations, University of Palermo, Palermo, May 15 - 17, 2019 (Contributed talk)
  \item "The polluted atmosphere as a shallow domain", Women in applied and computational Mathematics, Gran Sasso Science Institute, L'Aquila, May 9 - 11, 2018 (Poster presentation)
\end{itemize}
...
\begin{itemize}
  ...
  \item D. Donatelli, N. Juh\'{a}sz, Weak solution of the merged mathematical equations of the polluted atmosphere, Submitted Preprint 2019, arXiv:1906.09914 [math.AP]
