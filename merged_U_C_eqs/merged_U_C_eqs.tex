...
The energy inequality (\ref{energyIEQ}) now takes the form
  ...
  \begin{prop}
    Let $u^\epsilon_1, u^\epsilon_2, u^\epsilon_3$ and $C^\epsilon$ be the weak solutions of (\ref{anisotropicWF}). Then the sequences
  ...
  \begin{center}
    ...
    $u^\epsilon_3$ & is bounded in $L^2 (0,T; L^2(\Omega)),$ & \parbox{2.8cm}{\begin{equation} \label{propositionExtra} \end{equation} }\\
...
\begin{center}
    \begin{tabular}{r l l}
    $ \mathbf{u}_H^\epsilon \rightharpoonup \mathbf{u}_H $ & weakly in $L_t^\infty L_x^2 \cap L_t^2 H_x^1$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvUHEps} \end{equation} }\\
    $ \epsilon u_3^\epsilon \rightharpoonup f $ & weakly for some limit element $f$ in $L_t^\infty L_x^2 \cap L_t^2 H_x^1$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvU3Eps} \end{equation} }\\
    $ u_3^\epsilon \rightharpoonup u_3 $ & weakly in $L_t^2 L_x^2$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvU3NoEps} \end{equation} }\\
    $C^\epsilon \rightharpoonup C$ & weakly in $L_t^\infty L_x^2$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvCEps1} \end{equation} }  \\
    $\partial_2 C^\epsilon \rightharpoonup \partial_2 C $ & weakly in $L_t^2 L_x^2$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvCEps2} \end{equation} }\\
    $\partial_3 C^\epsilon \rightharpoonup \partial_3 C $ & weakly in $L_t^2 L_x^2$  & \parbox{2.4cm}{\begin{equation} \label{WeakConvCEps3} \end{equation} }\\
    $\sqrt{\epsilon} \partial_1 C^\epsilon \rightharpoonup e$ & weakly for some limit element $e$ in $L_t^2 L_x^2$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvCEps4} \end{equation} }\\
    \end{tabular}
\end{center}

\subsection{Passing to the limit}

    Taking the limit for the linear velocity terms that do not include the concentration function $C^\epsilon$ we can use (\ref{WeakConvUHEps}), (\ref{WeakConvU3Eps}) and (\ref{WeakConvU3NoEps}), to obtain weak convergences which are sufficient to pass to the limit in the linear expressions in (\ref{anisotropicWF}). As for the nonlinear terms, the convergence can be shown using a compactness theorem by \cite{simon1986compact}. The details are omitted since they are analogous to those described in \cite{azerad2001mathematical}.
...
By analyzing this nonlinear term in a more precise way we see that $(u_i C^\epsilon, \partial_i \tilde{C})$ for $i=1,2$ is weakly convergent since from \cite{azerad2001mathematical} we have the strong convergence of the horizontal velocities, while in the case of $i=3$ this property does not hold for $u_3$ and from the energy bound we do not have more than the weak convergence of $C^\epsilon$ .
  ...
  What we will show in the following is that for $h^\epsilon = u^\epsilon_i$ and $g^\epsilon = C^\epsilon$ the assumptions of Lemma \ref{PLL} are satisfied, which means that $u_i^\epsilon C^\epsilon \rightharpoonup  u_i C$ in the sense of distribution.
     ...
     To estimate the time derivative of $C^\epsilon$ we use (\ref{ANSC5}), that since the $u^\epsilon$ is divergence-free can be rewritten as
...
In order to be consistent with the notations of \cite{temam2005some}, throughout this section we will use the $V = V_1 \times V_2, H = H_1 \times H_2, \mathcal{V} = \mathcal{V}_1 \times \mathcal{V}_2,$ and $V_{(2)}$ function spaces --- they are defined analogously.

The scalar products and norms in the case of the polluted atmosphere are defined as follows.
...
\begin{definition}
Given a source term $S = \delta$ or $S \in L^2(\Omega),$ find $U = (\mathbf{v}, C),$ that satisfies the conditions of (\ref{hydrostaticWF}) in the space dimensions, moreover
...
\[ <AU, \tilde{U} > = \int_0 ^T a(U, \tilde{U}) \diff t, \quad \forall U, \tilde{U} \in V.\]
...
inner product in terms of operators. In the original version $\frac{\diff U}{\diff t}$ was used as an operator acting on $\tilde{U}$, the result defined to be their inner product. In our case, since we apply the weak formulation in space-time, and the time derivation is applied on the test function, we introduce the following operator $Z$:
...
\textbf{Step 5.} Finally we discuss the details of the time-dependent case, where we establish the existence, for all time, of the solutions for the system (\ref{hydrostaticWF}).

The weak formulation in the time-dependent case takes the following form.

\begin{definition}

Given $T,$ $U_0 \in H,$ and a source term $S = \delta$ or $S \in L^2(\Omega),$ find $U$ that satisfies the conditions of (\ref{hydrostaticWF}), moreover

\begin{equation} \label{timedependentWFE}
  \begin{split}
   \int_0^T (U, \frac{\diff}{\diff t} \tilde{U}) &+ a(U, \tilde{U}) + b(U,U, \tilde{U})+ e(U, \tilde{U}) \diff t =\\
   \int_0^T l(\tilde{U}) \diff t & + (U_0, \tilde{U}(0)), \quad \forall \tilde{U} \in V_{(2)}\\
 \end{split}
\end{equation}
...
\begin{equation}
  \begin{split}
    ...
    & + e(U^n, \tilde{U}) = l^n(\tilde{U}), \quad \forall \tilde{U} \in V_{(2)}. \\
  \end{split}
\end{equation}

We use $- \frac{1}{\Delta t} (U^n - U^{n-1}, \tilde{U})_H$ to approximate $(U, \frac{\diff}{\diff t} \tilde{U}),$ and note that we will have a minus sign difference from the original version --- as seen in the above expression --- since we apply the time derivative on the test function.
