...
\end{figure}
  ...
  \begin{itemize}
    ...
    \begin{equation}
      \mathbf{u}_H = 0, \quad u_3 = 0, \quad C = 0 \quad \text{ on } \Gamma_U \times (0,T)
    \end{equation}
      
    \item On $\Gamma_L$ we use the same boundary conditions as on the upper level, i.e. we have
    
    \begin{equation}
      \mathbf{u}_H = 0, \quad u_3 = 0, \quad C = 0  \quad \text{ on } \Gamma_L \times (0,T).
    \end{equation}
    ...
    \begin{equation}
      \mathbf{u}_H = 0, \quad u_3 = 0 \quad \text{ on } \Gamma_G \times (0,T) .
    \end{equation}
...
\section{Proof of the main theorem} \label{proof}
 ...
 This section is devoted to the proof of Theorem \ref{maintheorem}. In the extended case with the pollution equation the proof relies on a priori estimates gained from the energy inequality and, as we will see this is sufficient to pass to the limit in the linear terms. Concerning the nonlinear terms that do not include pollution we will follow \cite{azerad2001mathematical}. What we need to work on is how to handle the nonlinear terms including the new function $C^\epsilon.$ The challenge caused by these nonlinear terms will be met by proving that a compactness result from \cite{lions1996mathematical} is applicable to handle them.
  ...
  By using the generalized Young inequality for any $\zeta > 0,$ we can estimate this term in the following way.
...
The energy inequality (\ref{energyIEQ}) now takes the form
  ...
  \begin{equation}
    ...
    & + (K_3 - \zeta \frac{\epsilon}{2} K_1 c_T (c_S + 1)) \norm{\partial_3 C^\epsilon}_{L^2}^2 \} \diff \tau \leq \frac{1}{\zeta} K_1 \frac{\epsilon}{2} \int_0 ^ t \norm{\partial_1 C^\epsilon}^2 _{L^2 (\Gamma_G)} + \\
...
\begin{center}
    \begin{tabular}{r l l}
    $ \mathbf{u}_H^\epsilon \rightharpoonup \mathbf{u}_H $ & in $L_t^\infty L_x^2 \cap L_t^2 H_x^1$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvUHEps} \end{equation} }\\
    $ \epsilon u_3^\epsilon \rightharpoonup f $ & for some limit element $f$ in $L_t^\infty L_x^2 \cap L_t^2 H_x^1$ & \parbox{2.4cm}{\begin{equation} \label{WeakConvU3Eps} \end{equation} }\\
...
\subsection{Passing to the limit}
    ...
    Taking the limit for the linear velocity terms that do not include the concentration function $C^\epsilon$ we can use (\ref{WeakConvUHEps}), (\ref{WeakConvU3Eps}), the zero divergence condition of $\mathbf{u}^\epsilon,$ and the Poincare inequality to obtain weak convergences which are sufficient to pass to the limit in the linear expressions in (\ref{anisotropicWF}). As for the nonlinear terms, the convergence can be shown using a compactness theorem by \cite{simon1986compact}, and exploiting the $H_0(\partial_3, \Omega)$ regularity of $u_3$ provided by the zero divergence condition. The details are omitted since they are analogous to those described in \cite{azerad2001mathematical}.
...
\begin{theorem}
  Let $\mathbf{u}_0$ $\in$ $L^2 (\Omega)^3$, with $\nabla \cdot \mathbf{u}_0 = 0, \mathbf{u}_0 \cdot \mathbf{n} = 0$ on $\partial \Omega$; $C(0)\in$ $L^2 (\Omega)^3$, $C(0) = 0$ on $ \partial \Omega$; let $\theta_1, \theta_2 \in L^2 (0, T; H^{-1/2}(\Gamma_I))$ and $\sigma \in L^2 (0, T; H^1 (\Gamma_G))$; then as the aspect ratio $\epsilon$ tends to zero, any weak solution $(\mathbf{u^\epsilon}, C^\epsilon)$ of the coastal anisotropic equations (\ref{coastalAnisotropic}) converge to a weak solution ($\mathbf{u}$, C) of the hydrostatic coastal equations of the polluted atmosphere (\ref{coastalHydrostatic}).
\end{theorem}
