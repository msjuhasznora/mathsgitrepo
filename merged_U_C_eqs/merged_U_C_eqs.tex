...
\textbf{Step 2.} Now the hydrostatic weak formulation of the polluted atmosphere can be brought to an analogous format as seen in \cite{temam2005some}:
...
\begin{itemize}
  ...
  \item To prove the bound for $U, \tilde{U} \in V, U^\# \in V_{(2)}$, in this case we can not use the $L_\infty$ bound on the third term, since we would eventually arrive to $\big \| C^\# \big \| _{H^3}$, which means that we can not close the bound that way. We can use
  ...
  \item The establishment of the improvement $$ \lvert b(U, \tilde{U}, U^\#) \rvert  \leq c   \norm{U}   \lvert \tilde{U} \rvert _H^{1/2}     \big \| \tilde{U} \big \| ^{1/2}    \big \| U^\# \big \| _{V_{(2)}}     $$ of the first bound for $U, \tilde{U} \in V, U^{\#} \in V_{(2)}$, we consider the most typical term and we bound it by
  
  \[  \int_\Omega w \tilde{C} \nabla C^\# \diff \omega  \leq  \norm{w}_{L^2} \big \| \tilde{C} \big \| _{L^3} \big \| \nabla C^\# \big \| _{L^6}.\]
  
  Note that since our definition of the form $b$ is different from \cite{temam2005some}, thus we need to skip the switch of function arguments caused by the application of $ \lvert b(U, \tilde{U}, U^\#) \rvert = \lvert b(U, U^\#, \tilde{U}) \rvert$ in the original version of the proof, otherwise the bound can not be closed.
  ...
  \[ c \norm{\mathbf{v}} \big \| U^\# \big \| _{V_{(2)}} \lvert \tilde{U} \rvert ^ {1/2} \big \| \tilde{U} \big \| ^{1/2} ,\]
...
\begin{definition}
Given a source term $S$ in $L^2(\Omega),$ find $U = (\mathbf{v}, C),$ that satisfies the conditions of (\ref{hydrostaticWF}) in the space dimensions, moreover
...
\begin{theorem}
We are given $S$ in $L^2(\Omega),$ then the problem (\ref{FWSTAT}) possesses at least one solution $U \in V$ such that
...
The operators $A, B$ and $E$ can be set analogously as in the original version except for the additional integration because of the weak formulation we performed in the time dimension; $A$ for example is defined to be a linear continuous form from $V$ into $V',$ given by the formula

\[ <AU, \tilde{U} > = \int_0 ^T a(U, \tilde{U}) \diff t, \forall U, \tilde{U} \in V.\]
...
\[ \int_0 ^T (U, \frac{\diff}{\diff t} \tilde{U})_H \diff t \]

term in (\ref{EXISTENCEFORMAT}) in terms of operators. In the original version $\frac{\diff U}{\diff t}$ was used as an operator acting on $\tilde{U}$, the result defined to be their inner product. In our case, because we apply the weak formulation in space-time, the time derivation is applied on the test function, so we introduce the following operator $Z$:
...
\textbf{Step 5.} Finally we discuss the details of the time-dependent case, where we establish the existence, for all time, of the solutions for the system (\ref{hydrostaticWF}). Again, we point out that in this case, since in our version we performed the weak formulation in time as well, we can not use notations such as $\frac{\diff U_k}{\diff t}$. Instead, we will use $Z U_k$, which as an operator acting on $\tilde{U}$ that yields the inner product of the function and the time derivative of the test function.

The weak formulation in the time-dependent case takes the following form.

\begin{definition}

Given $T,$ $U_0 \in H,$ and a source term $S$ in $L^2(\Omega),$ find $U$ that satisfies the conditions of (\ref{hydrostaticWF}), moreover

\begin{equation}
  \begin{split}
   \int_0^T (U, \frac{\diff}{\diff t} \tilde{U}) &+ a(U, \tilde{U}) + b(U,U, \tilde{U})+ e(U, \tilde{U}) \diff t =\\
   \int_0^T l(\tilde{U}) \diff t & + (U_0, \tilde{U}(0)), \forall \tilde{U} \in V_{(2)}\\
 \end{split}
\end{equation}
...
\begin{equation}
  \begin{split}
    ...
    & + e(U^n, \tilde{U}) = l^n(\tilde{U}), \forall \tilde{U} \in V_{(2)}. \\
  \end{split}
\end{equation}
...
which finally leads us to the main existence result of this section.

\begin{theorem}

Given $T > 0$, $U_0 \in H,$ and a source term $S$ in $L^2(\Omega),$ then there exists $U$ which is solution to (\ref{hydrostaticWF}).
