...
\end{figure}
  ...
  \begin{itemize}
    ...
    while in the downwind (stronger) direction our assumption is more relaxed, we only assume that $\partial_1 C^\epsilon$ is finite in the boundary norm, i.e. in $L^2 L^2 (0,T; \Gamma_G)$.
...
In this section we describe the weak formulation of the merged equations in both the anisotropic and hydrostatic case we are going to use in the paper, and we state our main result.
  ...
  \begin{equation} \label{hydrostaticWF}
    \begin{split}
      & \int_0^T -(\mathbf{u}_H, \partial_t \mathbf{\tilde{u}}_H) + (\nabla_\nu \mathbf{u}_H, \nabla_\nu \mathbf{\tilde{u}}_H) - (\mathbf{u}_H, (\mathbf{u} \cdot \nabla) \mathbf{\tilde{u}}_H) + (b(\mathbf{u}_H), \mathbf{\tilde{u}}_H) \diff t \\
  ...
  \begin{equation} \label{anisotropicWF}
    \begin{split}
      & \int_0^T -(\mathbf{u}^\epsilon_H, \partial_t \mathbf{\tilde{u}}_H) + (\nabla_\nu \mathbf{u}^\epsilon_H, \nabla_\nu \mathbf{\tilde{u}}_H) - (\mathbf{u}^\epsilon_H, (\mathbf{u}^\epsilon \cdot \nabla) \mathbf{\tilde{u}}_H) + (b(\mathbf{u}^\epsilon_H), \mathbf{\tilde{u}}_H) \diff t \\
      & + \epsilon \beta \big[ \int_0 ^ T (u_3 ^\epsilon, \tilde{u}_1) - (u_1^\epsilon, \tilde{u}_3) \big] \diff t\\
      & + \epsilon^2 \big[ \int _0 ^ T -(u_3^\epsilon, \partial_t \tilde{u}_3) +(\mathbf{u}^\epsilon \cdot \nabla u_3 ^ \epsilon, \tilde{u}_3) + (\nabla_\nu u_3^\epsilon, \nabla_\nu \tilde{u}_3) \diff t\big] \\
...
Find $\mathbf{u}^\epsilon = (\mathbf{u}_H, u^\epsilon_3) \in L^2 (0,T ; \mathbf{V}) \cap L^\infty (0, T; L^2 (\Omega)^3)$ and $C^\epsilon$ with $C^\epsilon \in L^\infty L^2$, $C^\epsilon \in L^2 (0,T; H^1)$, $C^\epsilon(0) \in L^2_x$ such that
  ...
  \begin{equation} \label{coastalAnisotropic}
    \begin{split}
      ...
      & = - (\mathbf{u}^\epsilon_{0H}, \mathbf{\tilde{u}}_H(0)) - \int_0^T \langle \theta_H, \mathbf{\tilde{u}}_H \rangle_{\Gamma_I} - \epsilon^2 (u_{03}^\epsilon, \tilde{u}_3 (0)) - (C_0^\epsilon, \tilde{C} (0)) + \int_0^T (S^\epsilon,\tilde{C}) \diff t
    \end{split}
  \end{equation}
...
We denote by $U$ the vector $(\mathbf{u}_H, C)$ (three scalar functions). The scalar products and norms in the case of the polluted atmosphere are defined as follows.
