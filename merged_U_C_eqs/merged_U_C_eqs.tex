...
By analyzing this nonlinear term in a more precise way we see that $(u_i C^\epsilon, \partial_i \tilde{C})$ for $i=1,2$ is weakly convergent since from \cite{azerad2001mathematical} we have the strong convergence of the horizontal velocities, while in the case of $i=3$ this property does not hold for $u_3$ and from the energy bound we do not have more than the weak convergence of $C^\epsilon$ .
  ...
  What we will show in the following is that for $h^\epsilon = u^\epsilon_i$ and $g^\epsilon = C^\epsilon$ the assumptions of Lemma \ref{PLL} are satisfied, which means that $u_i^\epsilon C^\epsilon \rightharpoonup  u_i C$ in the sense of distribution.
     ...
     To estimate the time derivative of $C^\epsilon$ we use (\ref{ANSC5}), that since $u^\epsilon$ is divergence-free can be rewritten as
...
%%SUPERVISE, TEMPORARY. H is the space in where we take the inner product between dU / dt and tilde{U}, and where we take U_0 from.
$$ H_1 = \{ \mathbf{u}_H \in (L^2(\Omega))^2, \nabla \cdot \int_h^0 \mathbf{u}_H \diff z = 0, n_H  \cdot \int_h^0 \nabla \cdot \mathbf{u}_H \diff z = 0 \text{ on } \Gamma_L \}, $$
