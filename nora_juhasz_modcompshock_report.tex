...
\begin{frame}{Training activities in the academic year 2016/2017}
  ...
  \item
    \textbf{Gran Sasso Science Institute}: Introduction to Finite Elements Method, Decay property for hyperbolic systems with dissipation, Advanced Topics on Numerical Methods, Introduction to the incompressible Navier-Stokes equation
  \item
...
\begin{frame}{Research: Modeling of the atmosphere}
...
\begin{frame}{Synchronising}
One of the issues about synchronising was the fact that the equations of the atmosphere and pollution are often not in the same coordinate system.
For solving this we could either
\begin{itemize}
\item use Jacobians corresponding to the rescaled vertical coordinate in the equations, or
\item use the Jacobian of the spherical coordinate transform, or
\item simultaneously both of the above
\end{itemize}
...
\begin{frame}{Current state part 2}
 ...
 \[ \frac{\partial \bar{\rho_i}}{\partial t}  + \nabla_{h} \cdot (\bar{\rho_i} \bar{\bold{v}}) + \frac{\partial}{\partial z} (\bar{\rho_i} \bar{w})  -\bar{\rho} \nabla_{h} \cdot (K_{11}, K_{22}) \nabla \bar{Y_i} + \frac{\partial}{\partial z} (-\bar{\rho} K_{33} \frac{\partial \bar{Y_i}}{\partial z}) = \]
