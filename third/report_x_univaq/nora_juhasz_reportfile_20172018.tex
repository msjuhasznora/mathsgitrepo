...
One of the main tools that we use in order to exploit the "almost" two-dimensional structure of the atmosphere is to introduce the aspect ratio $$\epsilon = \frac{\text{characteristic depth}}{\text{characteristic width}},$$ which incorporates the shallowness in a mathematical way. This makes a natural scaling process possible: we use the $x=x_1, y=x_2, z=\epsilon x_3$ new coordinates on a new, $\epsilon$-independent domain. Correspondingly, we rescale the kinematic terms, the viscosity terms and the diffusivity coefficients as well.
...
\end{figure}
 ...
 Here we mention that the challenge of the local area model is to define open boundary conditions on the lateral boundary. We use a fix zero approach --- the concentration being set to zero actually stands for a constant concentration with the background flow being removed.
