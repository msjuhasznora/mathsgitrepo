...
In this section we describe the weak formulation of the merged equations in both the anisotropic and hydrostatic case we are going to use in the paper, and we state our main result.
  ...
  \begin{equation}
    \begin{split}
      ...
      &+ \int_0^T \bigg[ -(C, \partial_t \tilde{C}) -(\mathbf{u}C, \nabla \tilde{C}) +  (\mathbf{M} \nabla C, \nabla \tilde{C}) - \big[((\nabla \sigma)^ T \mathbf{M}) \tilde{C} \vec{\mathbf{1}} \big] _{\Gamma_G} \bigg ] \diff t \\
...
The energy inequality (\ref{energyIEQ}) now takes the form after applying the coercivity property (\ref{coercivity}) of the diffusion matrix.
  ...
  \begin{equation}
    ...
    & \leq \sum_{i,j = 1}^3 \frac{1}{\zeta} m_{ij} \frac{1}{2} \int_0 ^ t \norm{\sigma_i}^2 _{L^2 (\Gamma_G)} + \frac{1}{2} (\norm{\mathbf{u}_H^\epsilon (0)}_{L^2}^2 + \epsilon ^ 2 \norm{u_3^\epsilon (0)}_{L^2}^2 + \norm{C^\epsilon (0)}_{L^2}^2) \\
    & + \int_0^t (S^\epsilon,C^\epsilon) \diff \tau \raisetag{80pt} \label{finalformEIEQ}
    \end{split}
  \end{equation}  
...
By analysing this nonlinear term in a more precise way we see that $u^\epsilon_i C^\epsilon$ for $i=1,2$ is weakly convergent since as we mentioned before (\cite{azerad2001mathematical}) we have the strong convergence of the horizontal velocities. In the case of $i=3$ this property does not hold for $u_3$ --- for the vertical velocity we use the compact embedding $H^1 \hookrightarrow L^2$ and (\ref{WeakConvCEps1}). This provides
  ...
  which, using (\ref{WeakConvU3NoEps}), yields that the product $u^\epsilon_3 C^\epsilon$ is weakly convergent and thus, considering the previously mentioned results for $i=1,2$, we finally have $(\mathbf{u}^\epsilon C^\epsilon, \nabla \tilde{C}) \to (\mathbf{u} C, \nabla \tilde{C})$.
