...
\subsection{Passing to the limit}
    ...
    \[ \int_0 ^ T (\partial_2 C^\epsilon , \partial_2 \tilde{C}) + (\partial_3 C^\epsilon , \partial_3 \tilde{C}) d \mathrm{t} \to \int_0^T (\partial_2 C, \partial_2 \tilde{C}) + (\partial_3 C, \partial_3 \tilde{C}) \diff t.\]
      
    For the next term we have $\epsilon K_1 \int_0 ^T (\partial_1 C^\epsilon, \partial_1 \tilde{C}) = \sqrt{\epsilon} K_1 \int_0 ^T (\sqrt{\epsilon} \partial_1 C^\epsilon, \partial_1 \tilde{C})$, which goes to 0 as $\epsilon \to 0$ since we have $\sqrt{\epsilon} \partial_1 C^\epsilon$ bounded in $L^2_t L^2_x.$
      
    The $\epsilon K_1 \int_0^T \big[(\partial_1 C^\epsilon) \tilde{C} \big] _{\Gamma_G} \diff t$ line integral vanishes as $\epsilon \to 0$ because $\partial_1 C^\epsilon$ was assumed to be finite in norm on the lower boundary.
...
By analyzing this nonlinear term in a more precise way we see that $(u_i C^\epsilon, \partial_i \tilde{C})$ for $i=1,2$ is weakly convergent since as we mentioned before (\cite{azerad2001mathematical}) we have the strong convergence of the horizontal velocities, while in the case of $i=3$ this property does not hold for $u_3$ and from the energy bound we do not have more than the weak convergence of $C^\epsilon$ .
  ...
  What we will show in the following is that for $h^\epsilon = u^\epsilon_i$ and $g^\epsilon = C^\epsilon$ the assumptions of Lemma \ref{PLL} are satisfied, which means that $u_i^\epsilon C^\epsilon \rightharpoonup  u_i C$ in the sense of distribution.
     ...
     To estimate the time derivative of $C^\epsilon$ we use (\ref{ANSC5}), which since $\mathbf{u}^\epsilon$ is divergence-free can be rewritten as
         ...
         Now, the $S^\epsilon$ Gaussian source term is smooth; from (\ref{proposition1}), (\ref{propositionExtra}) we get $ \nabla \cdot (\mathbf{u}^\epsilon C^\epsilon) \in W_x^{-1,1}$ and from (\ref{proposition3}) we have that the remaining terms are in $W_x^{-1,2}.$ Hence we can conclude that $$\partial_t C \in L_t^1 W_x^{-1,1}.$$
...
\begin{remark}
However the results of this article were achieved in a classical, fixed Cartesian coordinate system, there are other possibilities to build up the model in. We will briefly consider two of these.

  \begin{enumerate}
  
    \item We fix the local Cartesian coordinate system to be adjusted to the downwind direction, i.e. $z$ is the vertical direction, while $x$ is the downwind, and $y$ is the crosswind direction. It is a legitimate assumption if we are considering a time interval which is not too long in the sense that it is reasonable to assume that we have a permanent, relatively stable main wind direction.  taking advantage of a downwind-direction matching coordinate system (\cite{goyal2011mathematical}).

  \end{enumerate} Wind-matching.
...
\[ ((C, \tilde{C}))_2 = \int _\Omega \big [ K_2 \partial_2 C \partial_2 \tilde{C} + K_3 \partial_3 C \partial_3 \tilde{C} \big ] \diff \omega \]
