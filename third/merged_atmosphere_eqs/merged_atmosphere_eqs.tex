...
\subsection{Passing to the limit}
    ...
    \[ \int_0 ^ T  \big[((\nabla C^\epsilon)^ T \mathbf{M}) \tilde{C} \vec{\mathbf{1}} \big] _{\Gamma_G} d \mathrm{t} \to \int_0^T \big[((\nabla C)^ T \mathbf{M}) \tilde{C} \vec{\mathbf{1}} \big] _{\Gamma_G} \diff t.\]
...
By analysing this nonlinear term in a more precise way we see that $u^\epsilon_i C^\epsilon$ for $i=1,2$ is weakly convergent since as we mentioned before (\cite{azerad2001mathematical}) we have the strong convergence of the horizontal velocities. In the case of $i=3$ this property does not hold for $u_3$ --- for the vertical velocity we use the compact embedding $H^1 \hookrightarrow L^2$ and (\ref{WeakConvCEps1}). This provides
  
  $$C^\epsilon \to C  \quad  \text{strongly in $L_t^2 L_x^2$,}$$
  
  which, using (\ref{WeakConvU3NoEps}), yields that the product $u^\epsilon_3 C^\epsilon$ is weakly convergent.
   
  This, combined with the previously described linear terms, concludes the proof of the main theorem.
