...
\begin{itemize}
  ...
  \begin{itemize}
     ...
     \item $ (\mathbf{M} \nabla C^\epsilon, \nabla \tilde{C}) \leq c \big \| \tilde{C} \big \| _{H^1} \| C^\epsilon \| _{H^1} \quad  \text{is bounded in $L^2(0,T)$,}$
  ...
  \item We apply the H\"older inequality and arrive to $$ \int_t^{t+h} \abs{\iota^\epsilon (s)} \diff s \leq c  \| \tilde{C}  \| _{H^2} h^{1/4}$$
...
\center{Note that the $\norm{\partial_1 C^\epsilon}$ term is replaced by $\textcolor{orange}{\| \sqrt{\epsilon} \partial_1 C^\epsilon \|.}$}
