...
\changefontsizes{8pt}
  ...
  \begin{equation}
    \begin{split} \nonumber
      ...
      & \hspace{-0.5cm}+ \epsilon \beta \int_0 ^ T \big [ (u_3 ^\epsilon, \tilde{u}_1) - (u_1^\epsilon, \tilde{u}_3) \big ] + \epsilon^2\big [ -(u_3^\epsilon, \partial_t \tilde{u}_3) +(\mathbf{u}^\epsilon \cdot \nabla u_3 ^ \epsilon, \tilde{u}_3) + (\nabla_\nu u_3^\epsilon, \nabla_\nu \tilde{u}_3) \big ] \diff t \\
...
As we have $C^\epsilon \in L^2_t H^1_x,$ we only need to verify $\norm{\tau_h C^\epsilon - C^\epsilon}_{L^2(0,T-h, {H^2}^*)} \leq \varphi(h) + \psi(\epsilon).$
\textbf{The main steps:}

\begin{itemize}
  \item Specifically, we prove $\norm{\tau_h C^\epsilon - C^\epsilon}_{L^2(0,T-h, {H^2}^*)} \leq c \cdot h^{1/4}$
...
\begin{itemize}
  \item $ (\tau_h C^\epsilon (t) - C^\epsilon (t), \tilde{C}) = \int_t^{t+h} \iota^\epsilon (s) \diff s, $ where we define $ \iota^\epsilon (s) = (\mathbf{u}^\epsilon C^\epsilon, \nabla \tilde{C}) - (\mathbf{M} \nabla C^\epsilon, \nabla \tilde{C}) + (S^\epsilon, \tilde{C}).$
  ...
  \begin{enumerate}
     ...
     \item $(\mathbf{u}^\epsilon C^\epsilon, \nabla \tilde{C}) \leq \norm{\mathbf{u}^\epsilon}_{L^2} \norm{C^\epsilon}_{L^3}  \big \| \tilde{C} \big \| _{H^2} \quad  \text{is bounded in $L^{4/3}(0,T),$} $ using the interpolation between $L_t^\infty L_x^2$ and $L_t^2 L_x^6$ for $C^\epsilon,$ obtaining that it is in $L_t^4 L^3_x$
  \end{enumerate}
  
  \item We apply the Holder inequality and arrive to $$ \int_t^{t+h} \abs{\iota^\epsilon (s)} \diff s \leq c  \| \tilde{C}  \| _{H^2} h^{1/4}$$
\end{itemize}
