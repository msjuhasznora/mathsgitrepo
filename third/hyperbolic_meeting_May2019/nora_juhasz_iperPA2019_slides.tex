...
\textbf{What theoretically should be used when modelling polluted wind flow.}

Classical / general concepts as foundation: a) The wind flow being GOVERNED by the incompressible Navier-Stokes equations, b) Using a convection - diffusion equation to describe the pollution effect

%% low Mach number: Boussinesq approximation, constant density. it is the conservation of mass eq. that becomes the div-free condition through the Boussinesq approximation.
...
Atmospheric and water flow are described by the 3D Navier-Stokes equations.

EQS

Note: full, complete. basic conservation of momentum.

\end{frame}

\begin{frame}

\textbf{What is actually used.}

A model widely used in meteorology, oceanology, limnology: the so-called primitive equations - the most natural simplification. Central component for describing the dynamics of geophysical fluids.

EQS

Note: in the vertical dimension we simply use the formula describing the hydrostatic pressure.

\end{frame}

\begin{frame}

A natural question to ask:  Is this okay? Why is this okay?
...
The KEY to the justification process is to take this phrase "we are considering a large-scale geophysical fluid on the surface of the Earth," and to include its meaning into the model in mathematical terms.

Scale analysis will show what are the dominant terms in the full vertical momentum equation: $\frac{\partial p}{\partial z},$ and $\rho g,$ resulting in the hydrostatic approximation. This approximate relation is highly accurate (shown by experience) for large-scale fluids and it is considered as a fundamental equation for oceanography and meteorology.

Our goal is to provide a rigorous analytical proof that this approximation is mathematically acceptable.
\end{frame}

\begin{frame}

The way we do this: we start by incorporating the concept of shallowness into the model and see what happens to the starting point model if we take the aspect ratio parameter closer and closer to 0 --- we will find that the two models are connected not just formally through this parameter, but also on the level of weak solutions.

Geophysical fluid $\to$ shallow domain characteristics --- the domain is greater horizontally than in the vertical dimension.

SCALING.

1. Dimensions and velocity.

2. Pressure and concentration.

3. Rescaling the diffusivity parameters according to the largely different horizontal and vertical scales. We highlight the fact that rescaling the coordinates and the velocity is not sufficient, and we need to consider the strongly anisotropic nature of viscosity and diffusivity.
...
%% formally get one set of equations from another set

%% highlight: both the starting model and the limit model are 3D, unlike in other approaches such as shallow water equations.

%% geophysical rotating frame: z, x, y

%% cartesian coordinates, local validity, beta plane - we concentrate our efforts to the corresponding beta-plane.
