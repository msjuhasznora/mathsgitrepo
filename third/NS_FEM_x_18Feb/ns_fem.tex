...
\title{Visualising weak convergence in a mathematical model in shallow geophysical domains}
\subtitle{A problem}
...
\begin{frame}{Anisotropic equations - Part 2}
  ...
  The initial and boundary conditions are chosen to be
    ...
    $$C^\epsilon = 0 \text{ on } ( \Gamma_U \cup \Gamma_L) \times (0,T),  \partial_3 C^\epsilon = 0 \text{ on } \Gamma_G \times (0,T),$$
    
    $$\partial_1 C^\epsilon, \partial_2 C^\epsilon \in L^2 L^2 (0,T; \Gamma_G). $$
...
\begin{frame}{Hydrostatic equations - Part 2}
    ...
    $$C = 0 \text{ on } ( \Gamma_U \cup \Gamma_L) \times (0,T),  \partial_3 C = 0 \text{ on } \Gamma_G \times (0,T),$$
    
    $$ \partial_1 C, \partial_2 C \in L^2 L^2 (0,T; \Gamma_G) $$
...
\begin{frame}{Some basic concepts - Part 1}

IPCS - Incremental pressure correction scheme. How to update $(\mathbf{u}^n, p^n)$ to arrive to $(\mathbf{u}^{n+1}, p^{n+1})$ without increasing both quantities at the same time.
...
\begin{frame}{Some basic concepts - Part 2}

Note that in the original weak formulation the $p$ pressure function is vanished as a consequence of the divergence-free condition. In this discretised scenario we don't exactly have that constraint fulfilled, so we also use the pressure function in the weak formulation $\to$ we define a mixed function space composed of
\begin{enumerate}
  \item a VectorFunctionSpace of continuous piecewise quadratics
  \item and a FunctionSpace of continuous piecewise linear functions.
\end{enumerate}
This mixed finite element space is known as the \textbf{Taylor-Hood} elements and is a stable, standard element pair for this kind of problem.

\end{frame}

\begin{frame}{Some basic concepts - Part 3}

Choice of element structure: the Taylor-Hood elements satisfy the BBL condition (coercivity plus an inf-sup condition on the functions $a$ and $b,$ respectively) that guarantees the well-posedness of the saddle point problem that we have:

$$ a(u,v) + b(v,\lambda) = <f,v>$$
$$ b(u, \mu) = <g, \mu> $$

We have this structure thanks to the remaining $\nabla p \cdot \mathbf{v}$ term in the Navier-Stokes equation and the $(\nabla \cdot \mathbf{u}) q = 0$ weak version of the divergence-free condition.
\end{frame}
